...
export const updateLikes = async (likedVideo, userId, type) => {
  console.log("update the following document:", likedVideo);
  ...
  try {
    ...
    if (type == "liked") {
      const updatedPost = await databases.updateDocument(
        ...
        likedVideo,
        {
          liked: [...doc.liked, userId],
        }
      );
      return updatedPost;
    } else {
      let updatedArray = doc.liked.filter((liked) => {
        liked.$id !== userId;
      });
      const updatedPost = await databases.updateDocument(
        databaseId,
